...
exports.Task = exports.TASK_STATUSES = void 0;
const EventEmitter_1 = require("../EventEmitter");
/**
 * Task statuses list
 */
exports.TASK_STATUSES = ['new', 'running', 'stopped', 'failed', 'done'];
/**
 * Default event debounce milliseconds
 */
let DEFAULT_EVENT_DEBOUNCE = 200;
/**
 ...
 * @returns `number`
 */
const _pos_num = (val, _default = 0, _blank = 0) => {
    if ([undefined, null].includes(val) || 'string' === typeof val && !(val = val.trim()))
        return _blank;
    if (!isNaN(val = parseFloat(val)) && Number.isFinite(val) && val >= 0)
        return val;
    return _default;
};
/**
 ...
 */
const _pos_int = (val, _default = 0, _blank = 0) => parseInt(_pos_num(val, _default, _blank) + '');
/**
 * Helper - round number
 *
...
const _get_str = (val) => {
    ...
    let value = '';
    try {
        if (Object(val) === val && val[Symbol.iterator])
            val = _json_str([...val]);
        value = String(val);
    }
    catch (e) {
    ...
    if (Object(val) === val && (!value || /\[object \w+\]/.test(value)))
        value = _json_str(val).replace(/^"|"$/g, '');
    return value.trim();
};
/**
 ...
 */
const _get_error = (val) => {
    if (Object(val) === val && !(val instanceof Error)) {
        if (val.error instanceof Error || 'string' === typeof val.error && !!val.error.trim())
            val = val.error;
        else if (val.reason instanceof Error || 'string' === typeof val.reason && !!val.reason.trim())
            val = val.error;
        else if ('string' === typeof val.message && !!val.message.trim())
            val = val.message;
    }
    return _get_str(val);
};
 ...
 */
const _debounce = (callback, timeout = 0) => {
    let timer, max_wait;
    const _handler = () => {
        clearTimeout(timer);
    ...
    return () => {
        ...
        if (!max_wait)
            max_wait = setTimeout(_handler, Math.floor(timeout * 1.5));
    };
};
/**
...
class Task {
     ...
     */
    constructor(name, linked = false, precision = Task.decimal_precision, event_debounce = Task.event_debounce) {
        if (!(name = _get_str(name)))
            throw new TypeError('Invalid new task name.');
        ...
        this[PROPS] = {
            ...
            item: undefined,
            _done: false,
            _round: (val) => _round(val, this[PROPS].precision),
            _emitter: new EventEmitter_1.EventEmitter(),
     ...
     * @returns `Task` instance
     */
    setProgress(progress, _value, _total) {
        const props = this[PROPS];
        //done - ignore updates
        if (props._done) {
        ...
        let tmp = _pos_num(progress, -1, -1);
        if (tmp < 0)
            throw new TypeError(`Invalid set task \`progress\` value (${progress}).`);
        ...
        if (!props.linked) {
            if ((_total = _pos_num(_total, -1, -1)) >= 0)
                total = props._round(_total);
            if ((_value = _pos_num(_value, -1, -1)) >= 0)
                value = props._round(_value);
        }
        else if (total) {
            if (progress === 100)
                value = total;
            else
                value = props._round(progress / 100 * total);
        }
        //changes - update
        let changes = 0;
        if (progress !== props.progress) {
    ...
    setTotal(total) {
        ...
        //parse total/adjust
        let tmp = _pos_num(total, -1, -1);
        if (tmp < 0)
            throw new TypeError(`Invalid set task \`total\` value (${total}).`);
        ...
        if (props.linked) {
            if (!total) {
                ...
                if (value >= total) {
                    ...
                    progress = props._round(value / total * 100);
            }
            else if (progress)
                value = props._round(progress / 100 * total);
        }
    ...
    setValue(value) {
        ...
        //parse value/adjust
        let tmp = _pos_num(value, -1, -1);
        if (tmp < 0)
            throw new TypeError(`Invalid set task \`value\` value (${value}).`);
        ...
        if (props.linked) {
            if (!value)
                ...
                total = props._round(100 / progress * value);
        }
        //changes - update
        let changes = 0;
        if (progress !== props.progress) {
     ...
     */
    static create(options, precision, event_debounce) {
        let { name, label, linked, precision: _precision, event_debounce: _event_debounce, progress, total, value, error, status, startTime, stopTime, complete, item, } = Object(options);
        try {
            //parse options
            let tmp;
            if (!(tmp = _get_str(name)))
                throw new TypeError('Invalid task `name` value.');
            name = tmp;
            ...
            if ((tmp = _pos_int(event_debounce, -1, tmp)) < 0)
                ...
                event_debounce = tmp;
            //-- precision round
            const _round_p = (val) => _round(val, precision);
            //-- parse/adjust: progress, total, value
            if ((tmp = _pos_num(progress, -1)) < 0)
                throw new TypeError('Invalid task `progress` value.');
            if ((progress = _round_p(tmp)) > 100)
                progress = 100;
            if ((tmp = _pos_num(total, -1)) < 0)
                throw new TypeError('Invalid task `total` value.');
            total = _round_p(tmp);
            if ((tmp = _pos_num(value, -1)) < 0)
                throw new TypeError('Invalid task `value` value.');
            value = _round_p(tmp);
            if (linked) {
                if (!value)
                    progress = 0;
                    ...
                    if (value > total) {
                        ...
                        if (progress !== prog) {
                            if (progress)
                                console.warn(`Task linked \`progress\` (${progress}) recalculated to (${prog}) using current value/total (${value}/${total}) %.`);
                            progress = prog;
                        }
                    }
                }
                else if (progress)
                    total = _round_p(100 / progress * value);
            }
            //-- parse/adjust: error, status, startTime, stopTime, complete
            complete = !!complete;
            error = _get_error(error);
            if (!(status = _get_str(status).toLowerCase()))
                status = 'new';
            else if (!exports.TASK_STATUSES.includes(status))
                status = 'new';
            if ((tmp = _pos_int(startTime, -1)) < 0)
                throw new TypeError('Invalid task \`startTime\` value.');
            startTime = tmp;
            if ((tmp = _pos_int(stopTime, -1)) < 0)
                throw new TypeError('Invalid task `stopTime` value.');
            stopTime = tmp;
            if (!(['stopped', 'failed', 'done'].includes(status) && startTime && stopTime && startTime < stopTime)) {
                ...
                complete = true;
            if (!exports.TASK_STATUSES.includes(status))
                throw new TypeError('Invalid task \`status\` value.');
            //create task
            ...
            props.event_debounce = event_debounce;
            props.progress = progress;
            props.total = total;
            props.value = value;
            props.error = error;
