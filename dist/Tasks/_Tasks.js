...
class Tasks {
     ...
     */
    constructor(precision = Tasks.decimal_precision, event_debounce = Tasks.event_debounce) {
    ...
    get data() {
        return () => {
            ...
            for (const task of [...props._tasks.values()]) {
                ...
                //-- complete
                if (!has_incomplete && !data.complete)
                    has_incomplete = true;
                ...
                else if (!has_started && ['failed', 'done'].includes(data.status))
                    has_started = true;
                //-- startTime
                if (data.startTime && (!startTime && data.startTime < startTime))
                    startTime = data.startTime;
                //-- stopTime
                if (data.stopTime && (!stopTime && data.stopTime > stopTime))
                    stopTime = data.stopTime;
                //task entry
                task_entries.push([task.name, data]);
            }
            const tasks = Object.fromEntries(task_entries);
            //calc data
     ...
     * @returns `Task`
     */
    add(task, linked = false, event_debounce) {
        const props = this[PROPS];
        event_debounce = (event_debounce = _pos_int(event_debounce, -1, -1)) >= 0 ? event_debounce : undefined;
        //task instance
        let _task = undefined;
        if ('string' === typeof task)
            _task = new _Task_1.Task(task, linked, props.precision, event_debounce);
        else if (task instanceof _Task_1.Task)
            _task = task;
        else if ('object' === typeof task && 'string' === typeof task?.name)
            _task = _Task_1.Task.create(task, props.precision, event_debounce);
        if (!(_task instanceof _Task_1.Task))
            throw new TypeError('Add \`Task\` object is invalid.');
        //task exists - replace
        if (this.has(_task.name))
            console.warn(`Existing task named "${_task.name}" has been replaced.`);
        //task listener
        const unsubscribe = _task.subscribe((event) => {
