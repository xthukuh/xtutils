 ...
 */
export type TEasing = {[key: string]: TEasingFunction};
 ...
 */
export const _tween = (name:TEasingKey, time: number, begin: number, change: number, duration: number): number => {
	if (!Easing.hasOwnProperty(name)) throw new TypeError(`Unsupported tween easing name (${name}).`);
	const x: number = time/duration*1, dx: number = Easing[name](x);
